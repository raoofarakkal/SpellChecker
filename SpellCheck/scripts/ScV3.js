var SpellChecker = (function (spellChecker) {
    ...
    spellChecker.SpellCheckForm = (function (spellCheckForm) {
        ...
        function SugContextMenu(pSpanObj) {
            ...
            mCm.attr("class", "Sc-ContextMenu");
            mCm.append("<div class=\"SpellcheckContextMenuItem\">Suggested words</div>");
            ...
            for (i in mSug) {
                ...
                if (mSug[i].toLowerCase() != "no suggestions found") {
                    mItems.push("<div class=\"wrap\"> <div class=\"SpellcheckContextMenuItem\" onclick=\"$('#divIframe iframe')[0].contentWindow.SpellChecker.SpellCheckForm.ScReplaceWord('" + pSpanObj.id + "','" + $(pSpanObj).text().replace(/\'/g, "\\'") + "','" + mSug[i].replace(/\'/g, "\\'") + "')\">" + mSug[i] + "</div> <div class=\"defineWord \" ><a href=\"#\">Describe</a><div class=\"arrow-down\"></div><div class=\"arrow-up\" style=\"display: none;\"></div></div></div><div class=\"defineWordContent\"></div>");
                } else {
                    mItems.push("<div class=\"Sc-ContextMenuItemNc\">No suggestions found.</div>");
                    mNoSug = true;
                }
            }
            mCm.append("<div class=\"SpellcheckContextMenuItemContainer\"><div class=\"spellCheckerWordContainer\">" + mItems.join("") + "</div></div>");
            mCm.append("<div class=\"hLine\"></div>");
            mCm.append("<div class=\"SpellcheckContextMenuItem\"><span onclick=\"$('#divIframe iframe')[0].contentWindow.SpellChecker.SpellCheckForm.ScAddToDic('" + $(pSpanObj).text().replace(/\'/g, "\\'") + "')\" style=\"cursor:pointer;color:#6186BD;\">Add <span style=\"font-weight:bold;color:red;\">" + $(pSpanObj).text() + "</span> to dictionary?</span></div>");

            mCm.append("<div class=\"hLine\"></div>");
            mCm.append("<div class=\"SpellcheckContextMenuItem\"><input id=\"scCustRepl\" value=\"" + $(pSpanObj).text() + "\" type=\"text\" style=\"height:16px !important;\"/>&nbsp;<input type=\"button\" value=\"Replace\" title=\"Replace and Add to dictionary\"  onclick=\"$('#divIframe iframe')[0].contentWindow.SpellChecker.SpellCheckForm.ScReplaceWord('" + pSpanObj.id + "','" + $(pSpanObj).text().replace(/\'/g, "\\'") + "', $('#scCustRepl').val())\"/></div>");
        ...
        spellCheckForm.showDefineWord = function (el) {
            $(el).find('.defineWord').show();
        }

        spellCheckForm.hideDefineWord = function (el) {
            $(el).find('.defineWord').hide();
        }

        spellCheckForm.showDefintion = function (el) {
        ...
        spellCheckForm.ScDefineWord = function (pWord) {
            if (pWord.length > 0) {
                $.ajax(
                    {
                        type: "POST"
                        , async: true
                        , url: "Collins.ashx"
                        , data: "cmd=searchfirst&word=" + pWord
                        , error: function (xhr, ajaxOptions, thrownError) {
                            alert('Failed to get definition for  \'' + pWord + '\' from collins dictionary');
                            //document.write('Error: Suggest.ashx ' + xhr.status + ' ' + thrownError + ' ' + xhr.responseText);
                        }
                        , success: function (response) {
                            if (response) {
                                if (response.Success) {
                                    if (response.Results) {
                                        var definition = JSON.parse(response.Results);
                                        $(".defineWordContent").html(definition.entryContent);
                                        $(".defineWordContent").append(definition.entryContent);
                                    }
                                }
                            }
                        }
